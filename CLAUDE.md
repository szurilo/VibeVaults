...
- **Tests**: Playwright E2E (`tests/`) — Tests across `access-matrix`, `account-deletion-safety`, `auth-roundtrip`, `dashboard`, `feedback-flow`, `member-departure`, `stripe-checkout`, `trial-expiration` with seed fixtures in `tests/fixtures/`. `workspace-paused.spec.ts` pins the owner-derived workspace gate across all three layers (routing, sidebar lock, and the mutation APIs), including that `/dashboard/account` stays reachable while locked out. `paywall-lock-sync.spec.ts` covers what the server-side gate alone gets wrong: the sidebar locking on the *same* redirected click rather than after a refresh, a voluntary upgrade visit keeping a usable nav, Create Workspace being locked for a lapsed owner but open for a never-owned member, and the exits on `/dashboard/settings/users` staying reachable. `marketing-header.spec.ts` pins the auth-aware chrome across all public pages (a break in the "signed in" direction would silently strip both conversion CTAs from logged-out visitors). `widget-network-capture.spec.ts` is the odd one out: a pure logic test that extracts the failed-request capture block straight out of `public/widget.js` and runs it against a stubbed window (no browser/DB/server), so the privacy guarantees on `/docs/widget-data` can't silently regress. `widget-pinning.spec.ts` needs a browser but no DB or dev server: `tests/utils/widget-harness.ts` serves the real `public/widget.js` against stubbed endpoints, covering anchor durability across viewport resizes (the failure mode is a pin drifting away from what it marks) and the on-page pin layer. **Zero retries** on CI and local — a flaky test is treated as a real bug, not noise to paper over. Any describe block that mutates shared owner state (billing, workspace membership) must save it in `beforeAll` and restore it in `afterAll`; missing restoration pollutes every subsequent test file. `global-setup.ts` writes a `.playwright-running` flag file that `src/lib/resend.ts` checks to short-circuit email sends during E2E runs (no dev inbox noise, no Resend quota burn).
- **CI/CD**: GitHub Actions — `deploy.yml` (production: `supabase db push` first, then a Vercel deploy hook; see Shipping Safety), `supabase-backup.yml` (backups), `playwright.yml` (E2E). **Vercel's own push-to-deploy is switched off for `main`** via `vercel.json` (`git.deploymentEnabled.main = false`), so production only builds when `deploy.yml` asks for it. Previews for PRs and `dev` still deploy automatically.
...
- **DB migrations** — the only category that can actively break live traffic. Use expand/contract: add column → deploy code that writes both shapes → backfill → deploy code that reads new → drop old. Never couple a destructive migration (drop/rename/NOT NULL without default) to a single deploy. `deploy.yml` guarantees the **ordering** (migration applied before the app is built, deploy skipped if the migration fails), so an additive migration and the code that uses it can ship in one PR. It does not make a destructive migration safe: the previous build keeps running until Vercel finishes, so a column it reads must still exist.
...
- **A workspace is live only while its OWNER pays — never gate on the viewer's own profile.** Members and clients hold no subscription of their own, so "can this person work here" is always a question about the workspace owner. Use `getWorkspaceAccess(workspaceId)` in `tier-helpers.ts` (or `getProjectWorkspaceAccess`), and for mutations the ready-made guards `checkWorkspaceActive` / `checkProjectWorkspaceActive` / `checkFeedbackWorkspaceActive`, which return `null` to proceed or a context-aware message (owners are told to renew, members to chase the owner). `checkOwnerAccess()` in `widget-helpers.ts` is a thin wrapper over the same helper, so the widget and the dashboard can never disagree.
  - This was a real revenue bug: the paywall used to read the *viewer's* profile and explicitly exempted invited workspaces, so when an owner's trial lapsed every member they had invited kept full access indefinitely. Pinned by `tests/workspace-paused.spec.ts` and the member cases in `tests/access-matrix.spec.ts`.
  - **Deliberately NOT gated**: leaving a workspace, removing a member, email preferences, and unsubscribe. Those are exits and opt-outs; trapping a user inside a workspace they can't use, or inside emails they can't stop, is a support and compliance problem rather than revenue protection. `/dashboard/account` is excluded from the routing gate for the same reason.
...
- **Stacking: the host's z-index is set inline with `!important`, never via `:host`.** Shadow DOM gives `:host` rules LOWER specificity than any rule in the customer's own stylesheet that matches the host element, so a page-level `div {}` or a framework reset can silently drop the entire widget behind the page (a Framer overlay did exactly this, covering the toast). `public/widget.js` therefore sets position/inset/z-index/pointer-events/display as inline `!important` on `#vibe-vaults-widget-host`, at `z-index: 2147483647`. Pinned by `tests/widget-stacking.spec.ts`, which mounts the widget on a deliberately hostile page.
- **Pinned feedback (the widget's only capture path)**: every report from the widget is anchored to a point on the page. Clicking anywhere drops a pin and opens a composer beside it; the panel in the corner holds the feedback list and threads. There is no unpinned "New" form any more — unpinned reports come from the dashboard.
  ...
  - **Reply pins (`1a`, `1b`…)**: a reply in a thread can carry its own `{ anchor, page_key }` (`feedback_replies.metadata`, whitelisted server-side by `pickReplyPinMetadata()`; a pin alone is a valid reply). The reply bar's pin button arms the *same* placement overlay as a new report (`armPin(feedbackId)` sets `pinTarget`), and the click lands on the reply being typed instead of opening the composer: amber pending marker that tracks its anchor, screenshot into the reply's attachments via the shared `captureScreenshotInto()` slot. **No chip in the reply bar** (it was noise for reviewers): the thumbnail is the pin's only handle there, and removing it drops the pin. The old reply-thread screenshot element picker was deleted; this replaced it. `/api/widget/feedback` projects anchored replies as an additive `pins: [{ reply_id, anchor, page_key, created_at }]` per thread, and just-sent ones are kept in `optimisticReplyPins` until echoed. The dashboard renders a `PinnedOnTile` ("Reply pin b") on anchored reply bubbles.
  - **Pin numbering is project-wide by age, not per page.** A reply pin can sit on a different page than its parent (deliberate: "same bug on /pricing too"), so it renders there alone as e.g. `4b`; per-page numbering would let that collide with `/pricing`'s own `1`. Only threads carrying a pin (own or reply) take a number. Caveat: hiding a completed report still shifts the numbers; a durable `feedbacks.pin_number` sequence is the fix if that ever matters (it would also let the dashboard show the number, which today only mirrors the letter).
...
- **The sidebar lock must be re-synced on soft navigations.** `/dashboard/subscribe` and `/dashboard/workspace-paused` share the `/dashboard` layout, and the App Router caches layout segments client-side — so a soft navigation the proxy redirects (a sidebar click by an owner whose trial just lapsed) swaps only the *page* segment and reuses the stale, pre-expiry sidebar. The nav looked fully enabled while every click bounced back to the paywall, until a manual refresh. `PaywallLockSync` fixes it: the page segment is always fresh, so it compares its own verdict against the layout's (handed down by `DashboardLockProvider`) and calls `router.refresh()` **only on mismatch** — refresh-on-mount would waste a round trip on every hard load, and a `setState` in an effect would trip this repo's `react-hooks/set-state-in-effect` rule. Every paywall-excluded route a locked-out user can land on must render it, with `locked` set from that route's own server-side verdict. On `/dashboard/subscribe` that is `isTrialExpired`, never hardcoded `true`: a healthy owner reaches it voluntarily from the footer's Upgrade link, and dimming their nav would strand them on the pricing page. Pinned by `tests/paywall-lock-sync.spec.ts`.
- **Exits are never gated, and they live on `/dashboard/settings/users`** — a member leaving, an owner removing a member or revoking a client. That route is excluded from the paywall and renders a **restricted mode** instead (roster and both exits live, inviting dropped, `POST /api/workspaces/invites` rejects it server-side anyway). Gating the page would trap a member inside a workspace that stopped working, the same failure mode as gating unsubscribe. `/dashboard/settings` (workspace settings) stays gated — the exclusion prefix is deliberately the longer path.
  - **The sidebar's `Users` item therefore stays interactive while the rest is dimmed**, alongside the workspace switcher. Both are exits. Reaching them only from a link on the paywall pages was the first attempt and it was wrong: people look for team and leave controls in the nav, so hiding them there hides them where nobody searches.
  - Dimmed groups carry **`data-locked="true"`**, and the specs assert against that rather than Tailwind class strings — which also means adding or removing a dimmed group can't silently invalidate a lock assertion.
- **"Create Workspace" is gated on the viewer's OWN plan, not the active workspace**: `trialStarted && isTrialExpired`. The distinction is load-bearing — `isTrialExpired()` is also true for an invited member who has never owned anything, and for them creating a first workspace is what *starts* their trial and is their only escape from someone else's lapsed workspace. A lapsed owner gets a lock icon routing to `/dashboard/subscribe` (routed, not disabled, so the reason is visible), plus a server-side guard in `createWorkspaceAction` — without it their null `subscription_tier` is read as Pro by `getTierLimits()` and the limit check alone would allow three more.
- **The tier badge and its Subscribe/Upgrade CTA are hidden whenever the active workspace is one the viewer does NOT own** — paused or healthy. The badge describes the *viewer's* billing, so in somebody else's sidebar it reads as that workspace's plan, which the viewer neither pays for nor can change. On a paused invited workspace the misreading is expensive: a Subscribe button beside "this workspace is paused" reads as "pay to unlock this workspace", and a member acting on it would be charged and see nothing change. On a healthy one it is merely false, which is reason enough — one rule covering both is easier to keep true than a rule that only applies once things break. Their own billing stays one workspace-switch away, and the switcher is deliberately always clickable, which is why hiding the CTA strands nobody (including a hybrid member whose own plan also lapsed: picking their own locked workspace in the switcher sets the cookie *and* routes to `/dashboard/subscribe`, so the context matches what they are paying for). The `activeWorkspace` guard on the flag keeps the CTA reachable for a viewer with no workspace at all. Pinned by the badge cases in `tests/paywall-lock-sync.spec.ts`.
- **`PaywallLockSync`'s `locked` is a question about the ACTIVE WORKSPACE, never about the viewer's trial.** The two come apart: a member whose own plan is healthy can sit on `/dashboard/subscribe` with a paused *invited* workspace still selected, and `isTrialExpired` would then wrongly report the sidebar as unlocked. Both `/dashboard/subscribe` and `/dashboard/settings/users` derive the flag the way `dashboard/layout.tsx` does — selected workspace, then `isWorkspaceLive()`.
- **Workspace paused page** (`/dashboard/workspace-paused`): the member-facing half of the paywall. Names the owner who has to renew, offers a `mailto:` nudge, reassures that nothing was deleted (the first thing support gets asked), and lists every other workspace of theirs that still works via `SwitchWorkspaceButton`. Self-heals: if nothing is actually paused it redirects to `/dashboard`, and an owner who lands there is forwarded to `/dashboard/subscribe`.
- **Sidebar lock is per-workspace, not per-viewer**: `dashboard/layout.tsx` resolves `lockedWorkspaceIds` from `getViewerWorkspaceLiveness()` and passes it to `AppSidebar` + `WorkspaceSwitcher`. The switcher shows a lock on *any* locked workspace (owned or invited) and routes a click to the right paywall. Default workspace selection prefers a **live** workspace, so a fresh login lands somewhere usable instead of on a paywall. The viewer's own `isTrialExpired` now drives only the tier badge and its CTA, which describe that user's billing rather than the active workspace.
...
- **Guest-safe email links**: `getNotificationPrefs()` returns a `recipientKind` (`'member' | 'client' | 'guest'`, resolved by `resolveRecipientKind()` in `notification-prefs.ts`) and every reply email passes it through. Only members get the `/api/email-redirect` "View in Dashboard" CTA — for clients and guests it lands on a login wall they can never pass, so they get the project's `website_url` instead (`threadCtaButton()`). The `/access` recovery line is hidden from guests entirely (`recoveryFooterLine()`): recovery enumerates profiles and client invites, so it can never find a review-link guest and would fail silently. Pinned by `tests/guest-email-links.spec.ts`.
...
| `embed-widget-card` | `src/components/embed-widget-card.tsx` | Widget embed snippet + `ActivateWidgetButton` |
| `activate-widget-button` | `src/components/activate-widget-button.tsx` | Shared "Activate widget" control (synchronous `window.open`, disabled until hydrated) |
...
| `pinned-on-tile` | `src/components/pinned-on-tile.tsx` | "Where on the page" tile shared by the feedback header and anchored reply bubbles |
| `switch-workspace-button` | `src/components/switch-workspace-button.tsx` | Escape hatch off `/dashboard/workspace-paused`; sets the cookie + hard-navigates so the proxy re-evaluates |
| `paywall-lock-sync` | `src/components/paywall-lock-sync.tsx` | Re-renders the cached dashboard layout when a paywall page's verdict disagrees with it, so the sidebar locks without a manual refresh |
| `dashboard-lock-context` | `src/components/dashboard-lock-context.tsx` | Forwards the layout's server-resolved lock state to client components (no state of its own, so no second source of truth) |
...
> Every workspace-scoped **mutation** (feedback status/delete/reply/manual-add, project delete, sharing toggle, review pause, `issueSelfWidgetLink`, `POST /api/projects`, `POST /api/workspaces/invites`, `/api/dashboard/upload` + `/confirm`) runs the owner-billing guard from `tier-helpers.ts` before writing. RLS only knows membership, never the owner's plan, and a dimmed sidebar does nothing for a tab that was already open when the lock landed. Both halves of the presigned-upload flow are gated: the PUT goes straight to Storage, so blocking only the presign would still let a confirm turn a stale URL into a row.
...
| `projects` | `id`, `name`, `api_key`, `workspace_id`, `website_url`, `share_token`, `is_sharing_enabled`, `review_token` (permanent, unique), `review_feedback_paused`, `widget_last_seen_at` (embed heartbeat; stamped by the **unauthenticated** `/api/widget/heartbeat` beacon that widget.js fires on every page load — the config GET also stamps it but only after a token authenticates, so it can never observe a freshly pasted snippet. Null ⇒ the project-settings page hides the Shareable Review Link card and the post-create dialog stays in its waiting state) |
| `feedbacks` | `id`, `project_id`, `content`, `type`, `sender`, `status`, `metadata` |
| `feedback_replies` | `id`, `feedback_id`, `content`, `author_role`, `author_name`, `metadata` (nullable reply pin `{ anchor, page_key }`) |
...
| `/api/widget/feedback` | GET | List feedback for widget (includes reply_count, `anchor` + `page_key` projected out of `metadata` for on-page pins, and `pins` = the thread's anchored replies) |
| `/api/widget/reply` | POST | Widget reply submission; optional `metadata: { anchor, page_key }` for a reply pin |
...
| `/api/widget/heartbeat` | POST | Unauthenticated beacon from widget.js on every page load; stamps `projects.widget_last_seen_at` so the dashboard can detect a freshly embedded snippet. Carries only the (public) API key |
...
- `/dashboard/settings/users` excluded from the **workspace paywall** (not from auth) — it hosts the only exits; see Dashboard Behaviours
- **Workspace paywall**, keyed on the selected workspace's **owner**, not the viewer: owner of a lapsed workspace → `/dashboard/subscribe` (they can pay); a member of one → `/dashboard/workspace-paused` (they can only nudge the owner, so pricing cards would be a dead end). The owner's billing row is invisible to members under `profiles` RLS, so the proxy reads it via the `get_user_workspace_billing()` SECURITY DEFINER RPC (migration `20260909000000`), which returns the **raw** columns so `hasActiveAccess()` stays the only implementation of the predicate. Fails open on RPC error — a hiccup must not lock paying customers out. `/dashboard/subscribe`, `/dashboard/workspace-paused`, `/dashboard/payment-success`, `/dashboard/account` and `/dashboard/settings/users` are excluded.
