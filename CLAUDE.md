...
- **The tier badge and its Subscribe/Upgrade CTA are hidden while viewing a locked workspace the viewer does NOT own.** The badge describes the *viewer's* billing, which is irrelevant there — and worse, a Subscribe button sitting beside "this workspace is paused" reads as "pay to unlock this workspace". A member acting on it would be charged and see nothing change, because their subscription cannot unlock a workspace somebody else owns. Their own billing stays one workspace-switch away, and the switcher is deliberately always clickable, which is why hiding the CTA strands nobody (including a hybrid member whose own plan also lapsed: picking their own locked workspace in the switcher sets the cookie *and* routes to `/dashboard/subscribe`, so the context matches what they are paying for).
