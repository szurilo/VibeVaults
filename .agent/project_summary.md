...
- **File Documentation**: For complex files (e.g., `feedback-card.tsx`), write a brief comment block at the top explaining their "Main Responsibility" and "Sensitive Dependencies".

## 4. Current Direction & Recent Epics
- **Workspace Architecture (Multi-Owned, Multi-Joined)**: Evolved the Workspace and Project hierarchy to allow agency users to create and own multiple workspaces securely (via RPC `create_workspace`), as well as delete their own workspaces. They can also join unlimited additional workspaces via invitation. Added cookie-based state persistence (`selectedWorkspaceId`, `selectedProjectId`) across the dashboard to navigate these environments.
- **Team Member & Client Invites**: Introduced a comprehensive invitation system supporting both `member` and `client` roles. Team members can be invited to a workspace to access all contained projects, while clients can be invited to review specific projects. Migrated RLS policies leveraging `SECURITY DEFINER` helper functions (like `get_client_project_ids()`) to securely allow invited clients to view and interact with feedbacks without fully joining the agency's primary workspace.
