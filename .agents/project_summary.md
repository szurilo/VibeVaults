...
- **Known browser bug — Firefox foreignObject rasterization (NOT FIXED)**: snapdom serializes the DOM into an SVG `<foreignObject>` and rasterizes it via `<img>`+canvas. On certain Firefox + GPU/driver combos with hardware acceleration enabled, Firefox's accelerated foreignObject paint drops `background-color` fills on small pill-shaped elements (`rounded-full` with non-transparent bg + box-shadow), leaving a ghost silhouette where only the box-shadow rendered. Reproduced on the pricing page's "Recommended" badge and "Monthly" toggle on one specific laptop; the visually-identical Subscribe button (`rounded-xl`) renders correctly. **Diagnostic chain**: identical `getComputedStyle` on both laptops → not a cascade issue; `about:support` Graphics rows + `gfx.webrender.software` toggle negative → not a WebRender tier issue; `Important Modified Preferences` empty diff → not an `about:config` pref; **Firefox Troubleshoot Mode disables the bug** → narrowed to "things Troubleshoot Mode disables"; flipping HW acceleration off in normal mode reproduces the fix → confirmed cause. Raw-SVG diagnostic (`window.snapdom.toRaw(document.body, ...)`) confirmed snapdom's serialization is correct (the `background-color: rgb(32, 156, 238)` declaration is in the SVG markup); Firefox's GPU rasterization pipeline is what drops the fill. **End-user workaround**: disable Firefox HW acceleration. **Code workarounds considered and rejected**: (1) pill-radius pre-pass (flatten `border-radius` to exact px and inline computed colors) — implemented and tested, **did not fix the bug** (because the bug is downstream of serialization); (2) canvas-substitute pre-pass (pre-rasterize problem pills via 2D canvas API and swap them in for capture) — would degrade screenshot fidelity for 100% of users to fix it for an unknown small minority, deferred until telemetry justifies the tradeoff; (3) switch to `html-to-image` — same library family (foreignObject), would have the same bug; (4) switch to `html2canvas` — only viable bypass but has its own significant fidelity tradeoffs (worse rendering of gradients, masks, modern CSS). **Telemetry shipped instead**: `/api/widget/screenshot-event` beacons each capture (success or failure) to PostHog with browser, UA, GPU vendor/renderer (via `WEBGL_debug_renderer_info`), DPR, viewport, and duration. After enough real-user data, decide whether canvas-substitute or library swap is justified.
...
| `/api/widget/screenshot-event` | POST | Receives screenshot-capture telemetry beacons → PostHog `widget_screenshot_capture` event (browser, GPU, DPR, viewport, duration). Used to size the Firefox foreignObject rasterization bug. |
